...
window.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`${window.location.href.toString()}.json`).then((data) => {
        data.json().then((dataInner) => {
            ...
            author.innerHTML = `${dataInner.author} | <small>Created: </small>${(new Date(dataInner.created * 1000)).toDateString()} | <small>Updated: </small>${(new Date(dataInner.updated * 1000)).toDateString()}`;
